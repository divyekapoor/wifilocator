...
/* Disable the scan shortcut hack */
#if 0
     ...
     char **	argv)
{
  ...
  /* do the actual work */
  /* Modification: Make sure you return with the ret_value of the called function */
  /* In case the devices have to be enumerated and run, return with the default last error code */
  if (dev)
    ret_value = (*iwcmd->fn)(skfd, dev, args, count);
  else
